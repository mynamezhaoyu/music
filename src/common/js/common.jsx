...
let obj = {
	...
	async update(index) {
		...
		if (process.env.TARO_ENV === 'weapp') {
			musicData = {
				...
				src: data.url || `https://music.163.com/song/media/outer/url?id=${data.id}.mp3`
			};
		} else {
			musicData = {
				autoplay: true,
	...
	async httpDetUrl(arr, type = true) {
		...
		let [ det, url ] = await Promise.all([ this.musicDetail(arr), this.musicUrl(arr) ]);
